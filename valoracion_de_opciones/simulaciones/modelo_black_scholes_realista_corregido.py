...
gaurdar_csv = False # True para guardar los datos en formato Excel (csv)
...
while True: #calulamos el valor de r anual
    try:
        entrada = input(f"Indica la comisión por volumen (0.0005 == 0.05% comision): \n")# nº de trayectorias a simular
...
while True: #calulamos el valor de r anual
    try:
        entrada = input("Indica el coste fijo por operación (1.25 == 1.25€ fijos por cada operación de compra/venta): \n")# nº de trayectorias a simular
...
trayectorias_delta_realista_corregido = [] # simula nuestra varacion de acciones real mediante bandas de tolerancia
...
# Cobertura realista (Bucle optimizado) || como ahora tengo matrices, no usamos bucles para ser mas eficientes
cartera_real_matriz = np.zeros_like(S_matriz)
cartera_real_matriz_corregido = np.zeros_like(S_matriz)

delta_realista_matriz = np.zeros_like(S_matriz)
delta_realista_matriz_corregido = np.zeros_like(S_matriz)
...
acciones_poseidas_corregido = np.full(n_trayectorias, delta0) # np.full(forma, valor)
coste_inicial = (np.abs(delta0) * posicion0 * comision_volumen) + coste_fijo
efectivo = V0_ajustado - (delta0 * posicion0) - coste_inicial
efectivo_corregido = V0_ajustado - (delta0 * posicion0) - (np.abs(delta0) * posicion0 * comision_volumen + coste_fijo) 

cartera_real_matriz[0] = V0_ajustado
cartera_real_matriz_corregido[0] = V0_ajustado

delta_realista_matriz[0] = delta0
delta_realista_matriz_corregido[0] = delta0

umbral = 5.5
contador_operaciones = np.ones(n_trayectorias)
...
for i in range(1, n_pasos): # ahora delta_teorico_t, ajsute_delta, coste_tx, efectivo, acciones_poseidas, son listas con las 100 trayectorias (ahorramos tiempo)
    ...
    efectivo_corregido = efectivo_corregido * np.exp(r * dt) # Interés del efectivo
    
    # if para rebalancear cada x pasos
    if i % frecuencia_rebalanceo == 0 or i == n_pasos - 1: #acutualizamos cada x pasos + el ultimo paso (lo usaremos para compararlo cuando lo corrigamos en el siguiente if)
    ...
    # ahora el "if" para corregir dependiendo de si el umbral es superado, como en cada matriz, cada fila(trayectorias) pueden una necesitar comprar y otras que no, usamos mascaras
    delta_teorico_t_corregido = Delta_matriz[i] # el corregido es solo para diferenciar que va a ir solo con el que vamos a corregir
    ajuste_delta_corregido = delta_teorico_t_corregido - acciones_poseidas_corregido

    valor_ajsute_corregido = np.abs(ajuste_delta_corregido)*S_t
    mascara_corregir = (valor_ajsute_corregido > umbral) | (i == n_pasos - 1) # nos devuelve true si es mayor a la umbral, o si es el ultimo paso, o False si es lo contrario, para cada trayectoria en el paso i || no pongo asytype(int) porque en los np.where() se necesita False o True
    
    coste_tx_corregido = (np.abs(ajuste_delta_corregido) * S_t * comision_volumen + coste_fijo)*mascara_corregir # coste total de las transacciones de compra/venta de acciones ()*0 si no se corrige y ()*1 si se corrige

    nuevo_efectivo = efectivo_corregido - (ajuste_delta_corregido * S_t + coste_tx_corregido)
    efectivo_corregido = np.where(mascara_corregir, nuevo_efectivo, efectivo_corregido) # evalua en cada trayectoria se es false o true, True ==> te da el nuevo efectivo, False ==> te devuelve efectivo_corregido, pero el de la anterior iteracion(se queda como estaba)

    acciones_poseidas_corregido = np.where(mascara_corregir, delta_teorico_t_corregido, acciones_poseidas_corregido) # lo mismo que en el efectivo pero para las acciones poseídas

    cartera_real_matriz[i] = (acciones_poseidas * S_t) + efectivo
    cartera_real_matriz_corregido[i] = (acciones_poseidas_corregido * S_t) + efectivo_corregido # nos serivirá para ver la diferencia entre poner y no poner costes fijos en los plots

    delta_realista_matriz[i] = acciones_poseidas
    delta_realista_matriz_corregido[i] = acciones_poseidas_corregido

    contador_operaciones = contador_operaciones + mascara_corregir.astype(int)
...
trayectorias_cartera_corregido = cartera_real_matriz_corregido.T.tolist()
trayectorias_delta_realista = delta_realista_matriz.T.tolist()
trayectorias_delta_realista_corregido = delta_realista_matriz_corregido.T.tolist()
...
############### Delta vs precio(realista corregido)
ax_deltas_realista_corregido = fig3.add_subplot(223)

# Plot de todas las trayectorias de las opciones
if tipo_opcion == "C":
    delta_inferior = ax_deltas_realista_corregido.axhline(0, color='red', linestyle='--', alpha=0.5, label = r"límite $\Delta$ inferior")
    delta_superior = ax_deltas_realista_corregido.axhline(1, color='green', linestyle='--', alpha=0.5, label = r"límite $\Delta$ superior")
else:
    delta_inferior = ax_deltas_realista_corregido.axhline(-1, color='red', linestyle='--', alpha=0.5, label = r"límite $\Delta$ inferior")
    delta_superior = ax_deltas_realista_corregido.axhline(0, color='green', linestyle='--', alpha=0.5, label = r"límite $\Delta$ superior")

cmap = plt.get_cmap('viridis') #viridis , winter, plasma, inferno, magma, cividis, Greys, Purples, Blues, Greens, Oranges, Reds, YlOrBr, YlOrRd, OrRd, PuRd, RdPu, BuPu, GnBu, PuBu, YlGnBu, PuBuGn, BuGn, YlGn
norm = colors.Normalize(vmin=t.min(), vmax=t.max())

todos_los_segmentos_real_corregido = []
todos_los_tiempos_real_corregido = []
paso_muestreo = 1 # Mantén el mismo que en el ideal para que sean comparables

for i in range(len(trayectorias_delta)):
    S_i = trayectorias[i]
    Delta_r_i = trayectorias_delta_realista_corregido[i] # ya no pongop el [::paso_muestreo] porque ahora solo se opera si se alcanza un umbral
    t_i = t
    ...
    todos_los_segmentos_real_corregido.append(segmentos)
    todos_los_tiempos_real_corregido.append(t_i[:-1])

# Esto convierte 100 listas en un solo bloque gigante de datos
final_segments_real_corregido = np.concatenate(todos_los_segmentos_real_corregido)
final_tiempos_real_corregido = np.concatenate(todos_los_tiempos_real_corregido)

# Colección única con todas las trayectorias
lc_real_corregido = LineCollection(final_segments_real_corregido, cmap='viridis', norm=norm, rasterized=False) #rasterized=True para que al cargar el png y moverte en el sea mucho mas rapido
lc_real_corregido.set_array(final_tiempos_real_corregido)
lc_real_corregido.set_linewidth(0.5)
lc_real_corregido.set_alpha(0.4)

# Añado solo una operación e dibujado
ax_deltas_realista_corregido.add_collection(lc_real_corregido)
...
# Graficamos curva teórica
curva_teorica, = ax_deltas_realista_corregido.plot(S_limite, deltas_teoricos, color='purple', alpha = 0.9, lw=2.5, label=r"$\Delta$ Teórico (Black-Scholes) para $t_v$ = T", zorder=100)# zorder=10 para que la curva teórica no se superponga sobre la curva de la cobertura
# Mediana del numero de operaciones
mediana_numero_operaciones = np.median(contador_operaciones)
media_numero_operaciones = np.mean(mediana_numero_operaciones)

#ajustes del subplot

ax_deltas_realista_corregido.set_title(rf"$\Delta$ en modelo realsita | Mediana de operaciones: {mediana_numero_operaciones:.0f} | Media de operaciones: {media_numero_operaciones:.2f}"                
                    f"\n Strike K={K}; Tasa r={r}; Volatilidad $\sigma$={sigma}")
ax_deltas_realista_corregido.set_xlabel("Precio (€)")
ax_deltas_realista_corregido.set_ylabel(r"$\Delta (acciones)$")
ax_deltas_realista_corregido.grid(True, alpha=0.3)

# legenda1
ax_deltas_realista_corregido.legend(handles=[delta_inferior, delta_superior, curva_teorica]) 

#mapa con colores 
sm = cm.ScalarMappable(cmap=cmap, norm=norm)
sm.set_array([]) # Esto es necesario para matplotlib

cbar = fig3.colorbar(sm, ax=ax_deltas_realista_corregido, pad=0.05)
cbar.set_label('Tiempo transcurrido (años)')

############### Delta vs tiempo (realista corregido)
ay_deltas_realista_corregido = fig3.add_subplot(224)

for i in range(len(trayectorias_delta)): 
    if i ==int(len(trayectorias_delta)/2):
        ay_deltas_realista_corregido.plot(t, trayectorias_delta_realista_corregido[i], lw=2, color='black', label="Trayectoria corregida arbitraria", zorder=10)
    else:
        ay_deltas_realista_corregido.plot(t, trayectorias_delta_realista_corregido[i], lw=0.5)

# mediana y media (mejo no lo pongo porque no repreetna nunguna estrategia real)
#mediana_delta_realista_corregido = np.median(trayectorias_delta_realista_corregido, axis=0)
#ay_deltas_realista_corregido.plot(t, mediana_delta_realista_corregido, color='black', lw=1, zorder=10, label = r"Mediana $\Delta$") # zorder=10 para que la linea de media no se superponga sobre la curva

# Añadimos líneas de referencia en 0 y 1 (límites de la Delta para Call)
if tipo_opcion == "C":
    ay_deltas_realista_corregido.axhline(0, color='red', linestyle='--', alpha=0.5, label = r"límite $\Delta$ inferior")
    ay_deltas_realista_corregido.axhline(1, color='green', linestyle='--', alpha=0.5, label = r"límite $\Delta$ superior")
else:
    ay_deltas_realista_corregido.axhline(-1, color='red', linestyle='--', alpha=0.5, label = r"límite $\Delta$ inferior")
    ay_deltas_realista_corregido.axhline(0, color='green', linestyle='--', alpha=0.5, label = r"límite $\Delta$ superior")

# ajustes
ay_deltas_realista_corregido.set_title(rf"$\Delta$ en modelo realsita | Mediana de operaciones: {mediana_numero_operaciones:.0f} | Media de operaciones: {media_numero_operaciones:.2f}"                
                    f"\n Strike K={K}; Tasa r={r}; Volatilidad $\sigma$={sigma}")
ay_deltas_realista_corregido.set_xlabel("Tiempo (años)")
ay_deltas_realista_corregido.set_ylabel(r"$\Delta (acciones)$")
ay_deltas_realista_corregido.grid(True, alpha=0.3)
ay_deltas_realista_corregido.legend(loc='best')
...
##### Modelo realista(costes totales) (sin corregir)
print("\n" + "="*50)
print("ANÁLISIS DE EFICIENCIA DE LA COBERTURA REALISTA (sin corregir)")
print("="*50)
...
##### Modelo realista corregido
print("\n" + "="*50)
print("ANÁLISIS DE EFICIENCIA DE LA COBERTURA REALISTA (corregido)")
print("="*50)

errores_realista_corregido = []
cartera_real_matriz_corregido = cartera_real_matriz_corregido[-1] #valores de cartera real sin coste fijo de cada trayectoria
...
errores_realista_corregido = cartera_real_matriz_corregido - payoff_real

# Estadísticas del error
error_mediana_realista_corregido = np.median(errores_realista_corregido)
error_medio_realista_corregido = np.mean(errores_realista_corregido)
error_std_realista_corregido = np.std(errores_realista_corregido)

print(f"Número de trayectorias: {n_trayectorias}")
print(f"Error medio de cobertura: {error_medio_realista_corregido:.4f} €")
print(f"Desviación típica del error: {error_std_realista_corregido:.4f} €")
print(f"Mediana del error: {error_mediana_realista_corregido:.4f} €")
print(f"El error máximo fue de: {np.max(np.abs(errores_realista_corregido)):.4f} €")
print("="*50 + "\n") # para seprar con ==============
...
##### Para el realista (corregido)
ax_res_realista_corregido = fig4.add_subplot(132)
ax_res_realista_corregido.scatter(S_finales[mask_itm], errores_realista_corregido[mask_itm], color='royalblue', edgecolors='black', label='ITM')
ax_res_realista_corregido.scatter(S_finales[mask_otm], errores_realista_corregido[mask_otm], color='gold', edgecolors='black', label='OTM')
ax_res_realista_corregido.axhline(0, color='red', linestyle='--', lw=2)
ax_res_realista_corregido.axhline(error_mediana_realista_corregido, color='purple', linestyle=':', label=f'Mediana: {error_mediana_realista_corregido:.2f}')
ax_res_realista_corregido.set_title("Modelo Realista con costes por volumen y costes fijos (corregido)")
ax_res_realista_corregido.set_xlabel("Precio final del activo (€)")
ax_res_realista_corregido.set_ylabel("Payoff neto de la cartera de cobertura (€)")
ax_res_realista_corregido.legend()

##### Para el realista (sin corregir)
...
ax_res_realista.set_title("Modelo Realista con costes por volumen y costes fijos (sin corregir)")
...
if gaurdar_csv: #cambiar la linea 20 (False ==> True) para guardar los datos en formato Excel (csv) | falta alguna cosa que añadir como lo no idea, pero como ya se sabe la semilla, no seria necesario pues el nº de trayectorias podria hacer los excels muy pesados
