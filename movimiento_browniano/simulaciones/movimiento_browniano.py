####################################################
# movimeinto_browniano.py
# Author: Mario Carracedo Andres
# Year: 2026
# License: MIT
# Description: Muestra el comportamiento aleatorio del movimiento browniano 
####################################################
...
for j in range(1, n_trayectorias+1):
    ...
    for i in range(1, n_pasos): # tiempo_maximo + 1 porque el tiempo 0 no se incluye en la lista
        ...
        #salto aletaroio entre posicion actual y la sieguiente en un dt
        #rn = random.random() # generamos un numero aleatorio entre 0 y 1 (no lo uso porque esto genera una distribucion uniforme, necesito normal)
        z = np.random.normal(0,1) # numero aletoario que sigue una distribucion normal, con media 0 y desviacion 1
                              # Partimo de una distribucion nomal estandar, porque luego la escalamos con sigma y mu para obtener el salto aleatorio deseado
        salto = (sigma*np.sqrt(dt))*z + (mu*dt) # sigma*raiz(dt)=desviación típica del salto aleatorio || mu*dt=tendencia del movimiento(mu te dice que de normal vas a hallar mu en 1s, pero si solo dejo un timepo dt para actuar, pues te mueves un dt*mu)
                                            # De normal, mu=0 y sigma=1 || con mu se suma porque te dice cuanto de nromal vas a hallar, por ende habra que sumarlo a nuestro salto general
                                            #                           || con sigma se multiplica porque te escala cuanto de grande es el salto
        # nueva posicion
        posicion_nueva = posiciones[-1] + salto # posicion ultima + salto aleatorio
...
fig_ax.set_title("Movimiento Browniano (Monte Carlo)\n"
                 f"Tendencia: mu={mu}; Volatilidad: sigma={sigma}; Semilla: {semilla}")
fig_ax.set_xlabel("Tiempo (s)")
fig_ax.set_ylabel("Posiciones (m)")
fig_ax.grid(True)
...
valor_esperado, = fig_ax.plot(t, np.array(t)*mu, lw=1, color = "red", label = "Valor esperado") # graficamos el valor esperado de la posición en cada tiempo
...
inferior = mu*t - 2*np.sqrt(t)*sigma # calculamos la zona inferior
superior = mu*t + 2*np.sqrt(t)*sigma # calculamos la zona superior
...
area_posible = fig_ax.fill_between(t, inferior, superior, color='purple', alpha=0.1, label="Zona de Confianza (95%)")

# legenda
fig_ax.legend(handles=[eje_x, valor_esperado, desviacion_tipca_up, desviacion_tipca_down, area_posible]) 
...
if mu == 0 and sigma == 1: #diferenciar si es estandar o aritmetico para el nombre del archivo
    nombre_archivo_imagen = "movimiento_browniano_estandar(png).png" #nombre de archivo a guardar como imagen
else:
    nombre_archivo_imagen = "movimiento_browniano_aritmetico(png).png" #nombre de archivo a guardar como imagen
...
if opcion_guardar == "y" or opcion_guardar == "Y":
    ...
    # Nombres de archivos
    if mu == 0 and sigma == 1: #diferenciar si es estandar o aritmetico para el nombre del archivo
        nombre_archivo_datos = "datos_movimiento_browniano_estandar(csv).csv" #nombre de archivo a guardar como datos
    else:
        nombre_archivo_datos = "datos_movimiento_browniano_aritmetico(csv).csv" #nombre de archivo a guardar como datos
